...
$query_rs_drug_indiv = "SELECT concat(h.hosptype,h.`name`) as hospname,t.among,d.pack_ratio FROM drug_take t left outer join drugs d on d.working_code=t.working_code and d.chwpart='".$_SESSION['chwpart']."' and d.amppart='".$_SESSION['amppart']."' left OUTER JOIN hospcode h on h.hospcode=t.hospcode WHERE SUBSTR(drug_take_date,1,7) ='".$_GET['month']."' and t.working_code='".$_GET['action_id']."' ".$condition." order by among DESC";
...
<table width="800" border="0" cellpadding="3" cellspacing="0" class="table_head_small table_collapse" >
  ...
  ?>
    ...
    <td width="57%" bgcolor="<?php echo $bgcolor; ?>" style="padding-left: 10px;border-left: 1px #BDD9FD solid;border-bottom: 1px #BDD9FD solid" ><?php echo $row_rs_drug_indiv['hospname']; ?>&nbsp;</td>
      <td width="23%" align="center" bgcolor="<?php echo $bgcolor; ?>"  style="border-left: 1px #BDD9FD solid; border-right: 1px #BDD9FD solid;border-bottom: 1px #BDD9FD solid" ><?php echo ($row_rs_drug_indiv['among']/$row_rs_drug_indiv['pack_ratio']); ?></td>
