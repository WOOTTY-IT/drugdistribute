...
$query_drug_roll = "SELECT d.hospcode,drug_take_date,count(*),concat(h.hosptype,h.name) as hospname,confirm,SUBSTR(drug_take_date,1,7) as 'month'  FROM drug_take d left OUTER JOIN hospcode h on h.hospcode=d.hospcode WHERE SUBSTR(drug_take_date, 1,7)=SUBSTR(CURDATE(), 1,7)  GROUP BY hospcode";
...
mysql_select_db($database_vmi, $vmi);
$query_drug_roll = "SELECT d.hospcode,drug_take_date,count(*),concat(h.hosptype,h.name) as hospname,confirm,SUBSTR(drug_take_date,1,7) as 'month' FROM drug_take d left OUTER JOIN hospcode h on h.hospcode=d.hospcode WHERE SUBSTR(drug_take_date, 1,7)='$monthyear' ".$condition." GROUP BY hospcode";
$drug_roll = mysql_query($query_drug_roll, $vmi) or die(mysql_error());
$row_drug_roll = mysql_fetch_assoc($drug_roll);
$totalRows_drug_roll = mysql_num_rows($drug_roll);
}
...
$totalRows_rs_date_period = mysql_num_rows($rs_date_period);
    ...
    <tr class="dashed">
      ...
      <td align="right"><a href="javascript:poppage('confirm_drug_take.php?confirm=<?php if($row_drug_roll['confirm']=="Y"){ echo "N"; } else{echo "Y"; } ?>&deptcode=<?php echo $row_drug_roll['hospcode']; ?>&month=<?php echo substr($row_drug_roll['drug_take_date'],0,7); ?>','300','210','','show','drug_roll_list.php','drug_roll_list','');" class="<?php if($row_drug_roll['confirm']=="Y"){ echo "small_red_bord"; } else { echo "small_blue2"; }?>">
      ...
      <td align="center"><?php if($row_drug_roll['confirm']=="Y"){?>
        <a href="javascript:poppage2('drug_take_roll_print.php?deptcode=<?php echo $row_drug_roll['hospcode']; ?>&month=<?php echo $row_drug_roll['month']; ?>','90%','90%');"><img src="images/Printer and Fax.png" width="27" height="27" border="0"></a>        <?php } ?></td>
    </tr>
