...
$query_rs_month = "select concat(t.year,'-',t.month) as monthyear from vaccine_take t left outer join hospcode h on h.hospcode=t.hospcode where h.chwpart='".$_SESSION['chwpart']."' and h.amppart='".$_SESSION['amppart']."' group by concat(t.year,'-',t.month) order by concat(t.year,t.month) DESC";
...
$thismonth=$row_drug_last['month'];

mysql_select_db($database_vmi, $vmi);
$query_rs_dept = "select d.hospcode from vaccine_take d left outer join hospcode h on h.hospcode=d.hospcode where d.hospcode !='' and h.chwpart='".$_SESSION['chwpart']."' and h.amppart='".$_SESSION['amppart']."' group by d.hospcode";
...
$(document).ready(function() {
    $('.link1').click(function(){
		...
		var dd=$('#dept').val();
		poppage2('vaccine_take_roll_indiv.php?month='+mm+'&deptcode='+dd,'90%','90%');
		});
});
</script>
</head>

<body onload="dosubmit('','show','vaccine_roll_list.php','drug_roll_list','');">

<form id="form1" name="form1" method="post" action="">
  <table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr>
    <td class="gray thsan-bold" style="font-size:20px; padding:10px" > ทะเบียนเบิกวัคซีนประจำเดือน <?php echo monthThai(date('Y-m')); ?></td>
  </tr>
    <tr>
      <td style="padding-left:20px"><span class="table_head_small_bord">ค้นหาทะเบียนการเบิกวัคซีน 
        &nbsp; </span>
...
?>
        ...
        &nbsp; <input type="button" name="search" id="search" value="ค้นหา" class="button gray2" onclick="dosubmit('','search','vaccine_roll_list.php','drug_roll_list','form1');" /></td>
    </tr>
    <tr>
