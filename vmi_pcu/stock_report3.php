...
mysql_select_db($database_vmi, $vmi);
$query_rs_pr = "select count(*) as countdrug,concat(DATE_FORMAT(receive_date,'%d/%m/'),(DATE_FORMAT(receive_date,'%Y')+543)) as receive_date,r.receive_no from stock_receive r left outer join stock_receive_c c on c.receive_no=r.receive_no where r.hospcode='".$hoscode."' group by r.receive_no order by receive_date DESC";
...
<body>
<form action="stock_report3.php" method="post">
  <div class="thsan-semibold" style="font-size:24px">ทะเบียนใบรับเวชภัณฑ์</div>
  <table width="600" border="0" cellspacing="0" cellpadding="5" class="thsan-light">
    <tr>
      ...
      <td width="432"><label for="dept"></label>
        <select name="dept" id="dept" class="thsan-light font16 inputcss1" onChange="window.location.href='stock_report3.php?dept_id='+this.value" >
          <?php
do {  
?>
...
?>
    ...
    <tr>
      <td>เลขที่รับ</td>
      <td><select name="pr" id="pr" class="thsan-light font16 inputcss1" >
        <?php
do {  
?>
        <option value="<?php echo $row_rs_pr['receive_no']?>"<?php if (!(strcmp($row_rs_pr['receive_no'], $_POST['pr']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rs_pr['receive_no']." ".$row_rs_pr['receive_date'];?></option>
        <?php
...
?>
  ...
  <div class="thsan-semibold font19">รับจากหน่วยงาน :<?php echo $row_rs_report['hospname']; ?></div>
