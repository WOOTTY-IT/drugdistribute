...
if($_GET['dept']==""){
    ...
    $condition=" and hospcode='".$_GET['dept']."'";
    $condition2=" and h.hospcode='".$_GET['dept']."'";
}
mysql_select_db($database_vmi, $vmi);
$query_rs_month_rate = "select count(*) as count_rate from month_rate where year_rate='".$_GET['year_rate']."' and month_rate='".$_GET['month_rate']."' and data_flag ='Y' ".$condition;
...
if($row_rs_month_rate['count_rate']>0){
	...
	mysql_select_db($database_vmi, $vmi);
	$query_delete = "delete from month_rate where month_rate='".$_GET['month_rate']."' and year_rate='".$_GET['year_rate']."'".$condition;
	$rs_delete = mysql_query($query_delete, $vmi) or die(mysql_error());

mysql_select_db($database_datacenter, $datacenter);
$query_rs_dbcenter = "select d.hospcode,d.did,sum(d.qty) as sumqty from datacenter_drug_opd d left outer join hospcode h on h.hospcode=d.hospcode where substr(vstdate,1,7)='".$_GET['year_rate']."-".$_GET['month_rate']."' and h.chwpart='".$_SESSION['chwpart']."' and h.amppart='".$_SESSION['amppart']."'".$condition2." GROUP BY d.hospcode,did";
