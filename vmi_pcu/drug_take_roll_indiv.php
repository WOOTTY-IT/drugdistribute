...
$query_rs_drug = "SELECT d.working_code,d.drug_name,FORMAT((sum(among))/pack_ratio,0) as sumamong,pack_ratio  FROM drug_take t left outer join drugs d on d.working_code=t.working_code and d.chwpart='".$_SESSION['chwpart']."' and d.amppart='".$_SESSION['amppart']."' WHERE SUBSTR(drug_take_date,1,7)='".$_GET['month']."' and d.item_type='".$_GET['item_type']."' ".$condition."  GROUP BY d.working_code ORDER BY d.drug_name";
...
<body>
<span class="big_black16">รายการยาเบิกแยกรายการ<?php echo $item_type_name; ?> ประจำเดือน<?php echo monthThai($month); ?></span>
<br />
<br />
